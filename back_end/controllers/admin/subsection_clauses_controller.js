...
export const Clause_By_SubSection= (req,res,next)=>{
    ...
    try {
        if(chapter_id && act_id&&section_id&&subsection_id){
            const query = `SELECT * FROM subsection_clauses WHERE chapter_id = ? AND act_id = ? AND section_id = ? AND subsection_id = ?`
...
export const Add_SubSection_Clause = (req,res,next)=>{
    ...
    if(act_id&&chapter_id&&section_id&&subsection_id){
        const query = 'INSERT into subsection_clauses (`act_id`,`chapter_id`,`section_id`,`subsection_id`,`clause_number`,`clause`) VALUES(?,?,?,?,?,?)'
    ...
    export const Update_SubSection_Clause = (req,res,next)=>{
        ...
        if(act_id&&chapter_id&&section_id&&subsection_id){
            try {
                const query = 'UPDATE subsection_clauses SET act_id=?,chapter_id=?,section_id = ?,subsection_id = ?, clause_number = ? ,clause = ? WHERE id = ?'
