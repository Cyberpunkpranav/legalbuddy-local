...
export const Sections_By_Chapter = (req,res,next)=>{
    ...
    try {
        if(chapter_id && act_id){
            const query = `SELECT * FROM sections WHERE chapter_id = ? AND act_id = ?`
...
export const Add_Section = (req,res,next)=>{
    ...
    if(act_id&&chapter_id){
        const query = 'INSERT into sections (`act_id`,`chapter_id`,`section_number`,`notified_date`,`section`) VALUES(?,?,?,?,?)'
        db.query(query,[act_id,chapter_id,section_number,notified_date,section],(err,result)=>{
            if(err){
                ...
                try {
                    ...
                    const query2 = 'Update sections set token = ? WHERE id=? '
    ...
    export const Update_Section = (req,res,next)=>{
        ...
        if(act_id&&chapter_id){
            try {
                const query = 'UPDATE sections SET act_id=?,chapter_id=?,section_number = ?,notified_date = ? ,section = ? ,token = ? WHERE id = ?'
