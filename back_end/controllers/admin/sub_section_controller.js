...
export const SubSections_By_Section= (req,res,next)=>{
    ...
    try {
        if(chapter_id && act_id&&section_id){
            const query = `SELECT * FROM subsections WHERE chapter_id = ? AND act_id = ? AND section_id = ?`
...
export const Add_SubSection = (req,res,next)=>{
    ...
    if(act_id&&chapter_id&&section_id){
        const query = 'INSERT into subsections (`act_id`,`chapter_id`,`section_id`,`subsection_number`,`subsection`) VALUES(?,?,?,?,?)'
    ...
    export const Update_SubSection = (req,res,next)=>{
        ...
        if(act_id&&chapter_id&&section_id&&chapter_id){
            try {
                const query = 'UPDATE subsections SET act_id=?,chapter_id=?,section_id = ?, subsection_number = ? ,subsection = ? WHERE id = ?'
