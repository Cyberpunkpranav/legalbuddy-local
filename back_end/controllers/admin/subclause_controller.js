...
export const SubClause_By_Clause= (req,res,next)=>{
    ...
    try {
        if(chapter_id && act_id&&section_id&&subsection_id&&clause_id){
            const query = `SELECT * FROM clause_subclauses WHERE chapter_id = ? AND act_id = ? AND section_id = ? AND subsection_id = ? AND clause_id = ?`
...
export const Add_SubClause = (req,res,next)=>{
    ...
    if(act_id&&chapter_id&&section_id&&subsection_id&&clause_id){
        const query = 'INSERT into clause_subclauses (`act_id`,`chapter_id`,`section_id`,`subsection_id`,`clause_id`,`subclause_number`,`subclause`) VALUES(?,?,?,?,?,?,?)'
    ...
    export const Update_SubClause = (req,res,next)=>{
        ...
        if(act_id&&chapter_id&&section_id&&subsection_id&&clause_id){
            try {
                const query = 'UPDATE clause_subclauses SET act_id=?,chapter_id=?,section_id = ?,subsection_id = ?, clause_id=?, subclause_number = ? ,subclause = ? WHERE id = ?'
